...
class UiAutomator2Server {
  ...
  async installServerApk (installRetries) {
    ...
    let retries = getRetries('Server install', installRetries, SERVER_INSTALL_RETRIES, 10000);

    logger.debug(`Waiting up to ${retries * 10000}ms for instrumentation '${INSTRUMENTATION_TARGET}' to be available`);
    let output;
    try {
      await retryInterval(retries, 10000, async () => {
